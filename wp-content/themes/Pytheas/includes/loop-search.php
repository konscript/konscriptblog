...
<article <?php post_class('search-entry clearfix'); ?>>  
    ...
    if($featured_image) {  ?>
        <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>" class="search-entry-img-link"><img src="<?php echo $featured_image; ?>" alt="<?php echo the_title(); ?>" /></a>
    <div class="search-entry-text">
    <?php } //featured image not set ?>
        <header>
            <h2><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a><span>(<?php echo get_post_type(); ?>)</span></h2>
       </header>
        	<p>
			<?php
            //show excerpt
