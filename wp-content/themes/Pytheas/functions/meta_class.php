...
class wpex_meta_box {
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'wpex_meta_box_nonce');
	...
	function show_field_taxonomy($field, $meta) {
		if (!is_array($meta)) $meta = (array) $meta;
		echo '<div class="wpex_tax_meta_wrap">';
		$this->show_field_begin($field, $meta);
	...
	function save($post_id) {
		...
		if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) // check autosave
			...
			|| (!check_admin_referer(basename(__FILE__), 'wpex_meta_box_nonce')) // verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// Validate meta value
			if (class_exists('wpex_meta_box_Validate') && method_exists('wpex_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('wpex_meta_box_Validate', $field['validate_func']), $new);
			}
